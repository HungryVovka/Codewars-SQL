...
-- returns films that have either "Deleted Scenes" or "Behind the Scenes" as a special feature, but not both - meaning that if there are "Deleted Scenes", 
-- there should not be "Behind the Scenes" and vice versa. The query should also exclude films that have "Commentaries" as a special feature.
-- 
-- Notes:
...
FROM film AS res
WHERE(
        special_features @> ARRAY['Behind the Scenes']::TEXT[] OR
        special_features @> ARRAY['Deleted Scenes']::TEXT[]
    ) AND NOT
    special_features @> ARRAY['Behind the Scenes', 'Deleted Scenes']::TEXT[] AND NOT
    special_features @> ARRAY['Commentaries']::TEXT[]
